...
def start(message):
    ...
    :return: None
    """
    bot.send_message(message.chat.id, 'Привет, я Russian Trip Bot! Я умею искать интересные места '
                                      'в городах России, которые ты мне напишешь 🗺')
...
def get_db(city):
    ...
    :return: data: Информация о городе из баззы данных
    """
...
def split_db(message, data):
    ...
    for i in data:
        text = f'<b>{i[2]}</b>\n\n{i[3]}\n\n<i>{i[4]}</i>'
        photo = i[1].split('; ')
        media = [telebot.types.InputMediaPhoto(open(f'{photo[0]}', 'rb'),
                                               caption=text, parse_mode='html'),
                 telebot.types.InputMediaPhoto(open(f'{photo[1]}', 'rb')),
                 telebot.types.InputMediaPhoto(open(f'{photo[2]}', 'rb'))]
...
def info(message):
    ...
    """
    city = message.text
    data = get_db(city)  # вся информация из бд
    url = 'https://ru.wikipedia.org/wiki/%D0%A1%D0%BF%D0%B8%D1%81%D0%BE%D0%BA_%D0%B3%D0%BE%D1%80%D0%BE%D0%B4%D0%BE%D0%B2_%D0%A0%D0%BE%D1%81%D1%81%D0%B8%D0%B8'
    df = pd.read_html(url)[0]
    if city in df.values:
        if not data:
            bot.send_message(message.chat.id, 'Такого города пока нет в моем '
                                              'списке 😔 Но в скором времени мы это исправим 😉')
        else:
